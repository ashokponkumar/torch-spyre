...
"""Parse GHA test-log output into hw_failure_diagnostics records.

Shared by every product repo's hardware-diagnostics ingest: one definition, so a fix here
reaches all of them. The CLI lives in each repo's .github/scripts/parse_hw_failures.py.

Hardware identifiers (node, PCI device, card serial, chip info) appear in the env dump of the
FINAL attempt only, so they are back-filled onto earlier attempts of the same suite. All RAS
events of an attempt are captured; the ras_* top-level fields reflect the earliest one.
"""
